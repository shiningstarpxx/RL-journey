...
class SARSA:
    ...
    - 在线策略学习 (on-policy)
    - 使用当前策略选择下一个动作
    - Q值更新: Q(s,a) ← Q(s,a) + α[r + γQ(s',a') - Q(s,a)]
    """
    ...
    def train_episode(self, env, max_steps: int = 1000) -> Tuple[float, int]:
        ...
        for step in range(max_steps):
            ...
            # 选择下一个动作 (SARSA的关键：使用当前策略)
            next_action = self.get_action(next_state_idx, next_valid_actions)
            ...
            action = next_action  # 使用选择的动作作为下一步的当前动作
    ...
    def train(self, env, num_episodes: int = 1000, max_steps_per_episode: int = 1000,
        ...
        """
        if verbose:
            pbar = tqdm(range(num_episodes), desc="Training SARSA")
        else:
    ...
    def render_policy(self, env, episode: int = None):
        ...
        if hasattr(env, 'render'):
            ...
            print(f"\nSARSA策略可视化 (Episode {episode if episode else 'Final'}):")
            print("=" * 50)
    ...
    def plot_training_history(self, save_path: Optional[str] = None):
        ...
        axes[0, 0].set_title('Episode Rewards (SARSA)')
        ...
        axes[0, 1].set_title('Episode Lengths (SARSA)')
        ...
        axes[1, 0].set_title('Epsilon Decay (SARSA)')
...
def test_sarsa():
    ...
    print(f"学习率: {sarsa.learning_rate}")
    print(f"折扣因子: {sarsa.discount_factor}")
    print(f"初始Epsilon: {sarsa.epsilon}")
    
    # 训练
    print("\n开始训练...")
    history = sarsa.train(env, num_episodes=500, verbose=True)
    
    # 评估
    print("\n评估结果:")
    eval_results = sarsa.evaluate(env, num_episodes=100)
    for key, value in eval_results.items():
        print(f"{key}: {value:.4f}")
    
    # 显示最终策略
    sarsa.render_policy(env, episode="Final")
    
    # 绘制训练历史
    sarsa.plot_training_history()
